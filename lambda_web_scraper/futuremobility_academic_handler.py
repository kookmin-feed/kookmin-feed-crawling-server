...
def handler(event, context):
    """
    미래모빌리티학과 학사공지 스크래퍼 Lambda Handler
    깔끔하고 독립적인 구현
    """
    print("🚀 [HANDLER] Lambda Handler 시작")
    try:
        # 동기 스크래퍼 실행
        scrape_futuremobility_academic()
        return {
            "statusCode": 200,
        }
        ...
        send_slack_notification(error_msg, "futuremobility_academic")
        return {
            "statusCode": 500,
        }

def scrape_futuremobility_academic() -> None:
    """
    미래모빌리티학과 학사공지를 스크래핑하고 새로운 공지사항을 처리
    """
    url = "https://cms.kookmin.ac.kr/futuremobility/board/notice.do"
    ...
    try:
        ...
        recent_notices = get_recent_notices("futuremobility_academic")
        ...
        saved_count = 0
        if new_notices:
            saved_count = save_notices_to_db(new_notices, "futuremobility_academic")
...
def parse_notice_from_element(element, kst, base_url) -> Dict[str, Any]:
    """HTML 요소에서 미래모빌리티학과 학사공지 정보를 추출"""
    try:
        # 상단 고정 공지 여부 확인
        ...
        result = {
            ...
            "scraper_type": "futuremobility_academic",
        }
        return result
    except Exception as e:
