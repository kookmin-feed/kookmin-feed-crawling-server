...
def handler(event, context):
    """
    교직과정부 학사공지 RSS 스크래퍼 Lambda Handler
    """

    print("🚀 [HANDLER] 교직과정부 학사공지 RSS 스크래퍼 Lambda Handler 시작")

    try:
        # 동기 스크래퍼 실행
        result = scrape_teaching_academic_rss()
        ...
        send_slack_notification(error_msg, "teaching_academic_rss")
        return {
            "statusCode": 500,
        }
...
def scrape_teaching_academic_rss() -> Dict[str, Any]:
    """교직과정부 학사공지 RSS를 스크래핑하고 새로운 공지사항을 처리"""

    url = "https://teaching.kookmin.ac.kr/introduce/notice/rss"
    kst = pytz.timezone("Asia/Seoul")

    print(f"🌐 [SCRAPER] 교직과정부 학사공지 RSS 스크래핑 시작 - URL: {url}")

    try:
        # 기존 공지사항 확인 (MongoDB에서)
        recent_notices = get_recent_notices("teaching_academic_rss")
        ...
        for entry in feed.entries[:20]:  # 최근 20개만 가져오기
            notice = {
                ...
                "scraper_type": "teaching_academic_rss",
            }
        ...
        saved_count = 0
        if new_notices:
            saved_count = save_notices_to_db(new_notices, "teaching_academic_rss")
            print(f"💾 [SCRAPER] 저장 완료: {saved_count}개")

        result = {
            "success": True,
            "message": "교직과정부 학사공지 RSS 스크래핑 완료",
