...
def handler(event, context):
    """
    차세대통신사업단 학사공지 스크래퍼 Lambda Handler
    깔끔하고 독립적인 구현
    """
    print("🚀 [HANDLER] Lambda Handler 시작")
    try:
        # 동기 스크래퍼 실행
        scrape_nccoss_general()
        return {
            "statusCode": 200,
        }
...
def scrape_nccoss_general() -> None:
    """
    차세대통신사업단 학사공지를 스크래핑하고 새로운 공지사항을 처리
    """
    url = "https://nccoss.kookmin.ac.kr/NCCOSS/community/notice.do"
    ...
    try:
        ...
        recent_notices = get_recent_notices("nccoss_general")
        ...
        saved_count = 0
        if new_notices:
            saved_count = save_notices_to_db(new_notices, "nccoss_general")
