...
def handler(event, context):
    """
    전자공학부 학사공지 RSS 스크래퍼 Lambda Handler
    """

    print("🚀 [HANDLER] 전자공학부 학사공지 RSS 스크래퍼 Lambda Handler 시작")

    try:
        # 동기 스크래퍼 실행
        result = scrape_creativeengineering_electrical_academic_rss()
...
def scrape_creativeengineering_electrical_academic_rss() -> Dict[str, Any]:
    """전자공학부 학사공지 RSS를 스크래핑하고 새로운 공지사항을 처리"""

    url = "https://ee.kookmin.ac.kr/community/board/notice/rss"
    kst = pytz.timezone("Asia/Seoul")

    print(f"🌐 [SCRAPER] 전자공학부 학사공지 RSS 스크래핑 시작 - URL: {url}")
    ...
    try:
        ...
        for entry in feed.entries[:20]:  # 최근 20개만 가져오기
            notice = {
                ...
                "scraper_type": "creativeengineering_electrical_academic_rss",
            }
        ...
        if new_notices:
            saved_count = save_notices_to_db(
                new_notices, "creativeengineering_electrical_academic_rss"
            )
            print(f"💾 [SCRAPER] 저장 완료: {saved_count}개")

        result = {
            "success": True,
            "message": "전자공학부 학사공지 RSS 스크래핑 완료",
