...
def handler(event, context):
    """
    경영대 학사공지 RSS 스크래퍼 Lambda Handler
    """
    ...
    try:
        ...
        send_slack_notification(error_msg, "businessadministration_academic_rss")
        return {
            "statusCode": 500,
...
def scrape_businessadministration_academic_rss() -> Dict[str, Any]:
    ...
    print(f"🌐 [SCRAPER] 경영대 학사공지 RSS 스크래핑 시작 - URL: {url}")

    try:
        # 기존 공지사항 확인 (MongoDB에서)
        recent_notices = get_recent_notices("businessadministration_academic_rss")
        ...
        if new_notices:
            saved_count = save_notices_to_db(
                new_notices, "businessadministration_academic_rss"
            )
            print(f"💾 [SCRAPER] 저장 완료: {saved_count}개")

        result = {
            "success": True,
            "message": "경영대 학사공지 RSS 스크래핑 완료",
