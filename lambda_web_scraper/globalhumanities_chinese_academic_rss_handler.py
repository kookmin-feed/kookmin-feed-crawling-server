...
def handler(event, context):
    """
    중국어문학부 학사공지 RSS 스크래퍼 Lambda Handler
    """

    print("🚀 [HANDLER] 중국어문학부 학사공지 RSS 스크래퍼 Lambda Handler 시작")

    try:
        # 동기 스크래퍼 실행
        result = scrape_globalhumanities_chinese_academic_rss()
        ...
        send_slack_notification(error_msg, "globalhumanities_chinese_academic_rss")
        return {
            "statusCode": 500,
        }
...
def scrape_globalhumanities_chinese_academic_rss() -> Dict[str, Any]:
    """중국어문학부 학사공지 RSS를 스크래핑하고 새로운 공지사항을 처리"""

    url = "https://cha.kookmin.ac.kr/china/china_notice/rss"
    kst = pytz.timezone("Asia/Seoul")

    print(f"🌐 [SCRAPER] 중국어문학부 학사공지 RSS 스크래핑 시작 - URL: {url}")

    try:
        # 기존 공지사항 확인 (MongoDB에서)
        recent_notices = get_recent_notices("globalhumanities_chinese_academic_rss")
        ...
        for entry in feed.entries[:20]:  # 최근 20개만 가져오기
            notice = {
                ...
                "scraper_type": "globalhumanities_chinese_academic_rss",
            }
        ...
        if new_notices:
            saved_count = save_notices_to_db(
                new_notices, "globalhumanities_chinese_academic_rss"
            )
            print(f"💾 [SCRAPER] 저장 완료: {saved_count}개")

        result = {
            "success": True,
            "message": "중국어문학부 학사공지 RSS 스크래핑 완료",
