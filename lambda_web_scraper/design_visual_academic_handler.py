...
def handler(event, context):
    """
    시각디자인학과 학사공지 스크래퍼 Lambda Handler
    깔끔하고 독립적인 구현
    """
    print("🚀 [HANDLER] Lambda Handler 시작")
    try:
        # 동기 스크래퍼 실행
        scrape_design_visual_academic()
        return {
            "statusCode": 200,
        }
        ...
        send_slack_notification(error_msg, "design_visual_academic")
        return {
            "statusCode": 500,
        }

def scrape_design_visual_academic() -> None:
    """
    시각디자인학과 학사공지를 스크래핑하고 새로운 공지사항을 처리
    """
    url = "https://vcd.kookmin.ac.kr/vcd/etc-board/vcdnotice.do"
    ...
    try:
        ...
        recent_notices = get_recent_notices("design_visual_academic")
        ...
        saved_count = 0
        if new_notices:
            saved_count = save_notices_to_db(new_notices, "design_visual_academic")
...
def parse_notice_from_element(element, kst, base_url) -> Dict[str, Any]:
    ...
    try:
        ...
        if relative_link.startswith("?"):
            link = f"https://vcd.kookmin.ac.kr/vcd/etc-board/vcdnotice.do{relative_link}"
        else:
            link = relative_link
        # 날짜 추출
        ...
        result = {
            ...
            "scraper_type": "design_visual_academic",
        }
        return result
    except Exception as e:
