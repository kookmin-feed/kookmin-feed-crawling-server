...
BASE_URL = "https://gds.kookmin.ac.kr/information/notice/"


def handler(event, context):
    """
    대학원 전체공지 RSS 스크래퍼 Lambda Handler
    """

    print("🚀 [HANDLER] 대학원 전체공지 RSS 스크래퍼 Lambda Handler 시작")

    try:
        # 동기 스크래퍼 실행
        result = scrape_graduate_totalnotice_rss()
        ...
        send_slack_notification(error_msg, "graduate_totalnotice_rss")
        return {
            "statusCode": 500,
        }
...
def parse_link(raw_link):
    ...
    (예: <link>4309</link> -> https://gds.kookmin.ac.kr/information/notice/4309)
    """
...
def scrape_graduate_totalnotice_rss() -> Dict[str, Any]:
    """대학원 전체공지 RSS를 스크래핑하고 새로운 공지사항을 처리"""

    url = "https://gds.kookmin.ac.kr/information/notice/rss"
    kst = pytz.timezone("Asia/Seoul")

    print(f"🌐 [SCRAPER] 대학원 전체공지 RSS 스크래핑 시작 - URL: {url}")

    try:
        # 기존 공지사항 확인 (MongoDB에서)
        recent_notices = get_recent_notices("graduate_totalnotice_rss")
        ...
        for entry in feed.entries[:20]:  # 최근 20개만 가져오기
            ...
            notice = {
                ...
                "scraper_type": "graduate_totalnotice_rss",
            }
        ...
        saved_count = 0
        if new_notices:
            saved_count = save_notices_to_db(new_notices, "graduate_totalnotice_rss")
            print(f"💾 [SCRAPER] 저장 완료: {saved_count}개")

        result = {
            "success": True,
            "message": "대학원 전체공지 RSS 스크래핑 완료",
