...
def handler(event, context):
    ...
    try:
        # 동기 스크래퍼 실행
        result = scrape_university_thursdaylecture()
        ...
        send_slack_notification(error_msg, "university_thursdaylecture")
        return {
            "statusCode": 500,
        }


def scrape_university_thursdaylecture() -> Dict[str, Any]:
    """
    목요특강을 스크래핑하고 새로운 공지사항을 처리
    """

    url = "https://www.kookmin.ac.kr/user/kmuNews/specBbs/thursdayLecture/index.do"
    ...
    try:
        ...
        recent_notices = get_recent_notices("university_thursdaylecture")
        ...
        saved_count = 0
        if new_notices:
            saved_count = save_notices_to_db(new_notices, "university_thursdaylecture")
            print(f"💾 [SCRAPER] 저장 완료: {saved_count}개")

        result = {
            "success": True,
            "message": f"목요특강 스크래핑 완료",
    ...
    try:
        ...
        if board_etc:
            ...
            if spans and len(spans) > 0:
                ...
                try:
                    # "2025.05.29 (오후 3시)" 형식에서 날짜 부분만 추출
        ...
        if "global.write(" in onclick:
            # onclick="global.write('59019', './view.do');" 에서 post_id 추출
            parts = onclick.split("'")
            if len(parts) >= 2:
                post_id = parts[1]
                link = f"https://www.kookmin.ac.kr/user/kmuNews/specBbs/thursdayLecture/view.do?dataSeq={post_id}"
            else:
                link = "https://www.kookmin.ac.kr/user/kmuNews/specBbs/thursdayLecture/index.do"
        else:
            link = "https://www.kookmin.ac.kr/user/kmuNews/specBbs/thursdayLecture/index.do"
        ...
        result = {
            ...
            "scraper_type": "university_thursdaylecture",
        }
