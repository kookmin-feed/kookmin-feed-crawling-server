...
def handler(event, context):
    """
    광고홍보학전공 학사공지 스크래퍼 Lambda Handler
    깔끔하고 독립적인 구현
    """
    ...
    try:
        # 동기 스크래퍼 실행
        result = scrape_socialscience_communication_advertising_academic()
        ...
        send_slack_notification(
            error_msg, "socialscience_communication_advertising_academic"
        )
        return {
            "statusCode": 500,
        }
...
def scrape_socialscience_communication_advertising_academic() -> Dict[str, Any]:
    """
    광고홍보학전공 학사공지를 스크래핑하고 새로운 공지사항을 처리
    """

    url = "https://adpr.kookmin.ac.kr/adpr/menu/undergraduate-notice.do"
    ...
    try:
        ...
        if new_notices:
            saved_count = save_notices_to_db(
                new_notices, "socialscience_communication_advertising_academic"
            )
            print(f"💾 [SCRAPER] 저장 완료: {saved_count}개")

        result = {
            "success": True,
            "message": f"광고홍보학전공 학사공지 스크래핑 완료",
...
def parse_notice_from_element(element, kst, base_url) -> Dict[str, Any]:
    ...
    try:
        ...
        if relative_link.startswith("?"):
            ...
            link = f"https://adpr.kookmin.ac.kr{relative_link}"
        else:
            link = f"https://adpr.kookmin.ac.kr/{relative_link}"
        ...
        if not date_span:
            # 첫 번째 방법으로 날짜를 찾지 못한 경우 테이블의 마지막 셀 확인
            date_td = (
                element.select("td")[-1] if len(element.select("td")) > 0 else None
            )
        ...
        result = {
            ...
            "scraper_type": "socialscience_communication_advertising_academic",
            "korean_name": "광고홍보학전공 학사공지",
        }
